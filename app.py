...
st.title("🖨️ पॅनेल मतदार स्लिप जनरेटर (A5 Portrait - Bottom Info Layout)")
...
def register_marathi_font():
    try:
        font_url = "https://github.com/googlefonts/noto-fonts/raw/main/hinted/ttf/NotoSerifDevanagari/NotoSerifDevanagari-Regular.ttf"
        font_data = urllib.request.urlopen(font_url).read()
        font_io = io.BytesIO(font_data)
        pdfmetrics.registerFont(TTFont('NotoSerifDevanagari', font_io))
...
if uploaded_file is not None:
    ...
    # ३. कॅनव्हास आधारित सुरक्षित पीडीएफ जनरेशन लॉजिक (A5 Portrait - Bottom Fixed Layout)
    def generate_a5_portrait_slips_canvas(data_frame, banner_bytes, polling_station):
        buffer = io.BytesIO()
        width, height = A5  # A5 Portrait परिमाणे
        c = canvas.Canvas(buffer, pagesize=A5)
        
        for index, row in data_frame.iterrows():
            # बाहेरील मुख्य आऊटलाईन बॉक्स (पूर्ण A5 वर कडक फिटिंग)
            c.setStrokeColorRGB(0, 0, 0)
            c.setLineWidth(2)
            c.rect(15, 15, width - 30, height - 30)
            
            # १. पॅनेल बॅनर - अगदी सर्वात वरती (Top ला) फिट करणे
            if banner_bytes:
                try:
            ...
            # ----------------------------------------------------
            # [BIG SPACE] - बॅनर आणि माहितीच्या मध्ये पूर्ण मोकळी जागा सुटेल
            # ----------------------------------------------------
            
            # २. कापावे रेष - लेआउटनुसार खाली (Bottom ला) शिफ्ट केली आहे
            c.setStrokeColorRGB(0.5, 0.5, 0.5) # डार्क ग्रे
            c.setLineWidth(0.5)
            c.setDash(4, 4)
            c.line(20, 175, width - 20, 175) # रेष तळाशी शिफ्ट केली
            c.setDash() # रिसेट डॅश
            
            c.setFont(font_name, 10)
            c.setFillColorRGB(0.4, 0.4, 0.4)
            c.drawCentredString(width / 2, 180, "----------------- मतदान केंद्रात जाण्यापूर्वी येथून कापावे -----------------")
            ...
            # ३. मतदाराची मुख्य माहिती - अगदी तळाशी (Bottom ला) सेट केली आहे
            c.setFillColorRGB(0, 0, 0)
            c.setFont(font_name, 14) # फॉन्ट साईझ खालच्या भागासाठी योग्य ठेवली आहे
            
            start_y = 155 # रेषेच्या बरोबर खालून माहिती सुरू
            line_gap = 20  # ओळींमधील व्यवस्थित अंतर
            ...
            c.drawString(35, start_y - (line_gap * 4), f"घर क्रमांक :  {house_no}            भाग नं: {part_no}")
    ...
    if st.button("🚀 नवीन A5 Portrait मतदार स्लिप्स पीडीएफ जनरेट करा"):
        ...
        with st.spinner("तुमच्या सांगण्यानुसार तळाशी माहिती असलेल्या स्लिप्स तयार होत आहेत..."):
            pdf_out = generate_a5_portrait_slips_canvas(df, banner_data, polling_station_input)
            
            st.success("🎉 सर्व स्लिप्स तुमच्या दुसऱ्या फोटोच्या लेआउटप्रमाणे तळाशी सेट झाल्या आहेत!")
            st.download_button(
